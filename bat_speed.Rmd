...
change_delta<-indiv_delta%>%
  ...
  ggplot()+
  geom_col(aes(x = bins, y = avg_delta, fill = avg_delta)) +  
...
avg_speeds_heights%>%
 ...
 group_by(height_group)%>%
  ...
  scale_x_continuous(breaks = seq(20,42,1)) + 
  labs(x = "Age", y = "Avg Change in Bat Speed (MPH)",
       title = "Average Change in Bat Speed (2015-2019)") + 
...
imputed<- mice(before_impute%>%select(-player_name, -three_fourths_t1, -avg_ev_t1, - Pos, -med_speed_t1, -weight))
